class UserSessionsController < Devise::SessionsController
  ...
  before_filter :find_session, :only => [:show_error, :destroy]
  ...
  if (request.url.include?(APP_CONFIG['domain_url']) || request.url.include?(APP_CONFIG['secure_domain_url']))
      ...
      elsif logged_in_user.store_owner && logged_in_user.store_owner.domain_customize.custom_domain == subdomain[0]
              (logged_in_user.is_owner?)? (redirect_to storeowner_url(:subdomain =>customize_domain.custom_domain,:user_id=>logged_in_user.id)) :(show_success_message)
              else
          show_error
      end
    end
